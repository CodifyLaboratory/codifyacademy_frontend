<script>
  ...
  language.subscribe(async lang => {
    activeLang = lang
    request('get', 'children-courses/').then(data => {
      currentCards = data || []
      filter(0)
  ...
  function filterCards(age_category) {
    request('get', 'children-courses/', { age_category }).then(data => {
      currentCards = data || []
      // if (length && data.length > length) {
      //   currentCards.length = length
      // }
